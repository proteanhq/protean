...
If you are running the [Observatory](server/observability.md), the live message
...
2. If the events have an outdated schema, consider writing an
   [upcaster](event-upcasting.md) to transform them to the current schema.
...
- [Priority Lanes](priority-lanes.md) -- Conceptual guide explaining how
  priority lanes work and when to use them.
- [Outbox Pattern](server/outbox.md) -- How the outbox ensures reliable
  message delivery.
- [Observability](server/observability.md) -- The Observatory dashboard and
