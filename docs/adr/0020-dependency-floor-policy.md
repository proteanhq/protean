...
1. **Protean tests on the newest supported Python (3.14)**: A declared floor
   ...
   below by the first version shipping a 3.14 (`cp314`) wheel, which, in mid 2026,
   is close to current. C-extension floors cannot be widened as far as
...
- **The floor-proving CI leg is the enforcement mechanism**: The `test` job runs
  ...
  application can present. It runs the full suite. A floor that stops
  resolving, loses its newest-Python wheel, or drifts out of API compatibility
  fails CI instead of rotting silently. (The normal legs resolve to latest via
  `uv.lock` and never exercise the floors; which is why the floors were free to
  drift.)
...
- **Keep `versioning-strategy: auto` and merge the bumps**: Rejected: it is the
  ...
  pins, which is exactly backwards: an application wants its lockfile current,
  a library wants its floors low.
