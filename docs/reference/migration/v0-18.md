...
0.18.0. It reports every String/Text field declared without a `sanitize=` kwarg,
by `module:line`, so you can decide each one. A field that already passes
`sanitize=True` or `sanitize=False` declared its intent and is not reported.
...
The domain-level `[field_defaults] sanitize` **is** that flag: set it to `true`
and every unset field sanitizes as before. The framework default flips to
`False` because storing an HTML-escaped value is the wrong default for a domain
model, and `bleach` on write is not the security control it looks like. The
upgrade check is the migration net: it enumerates the exact sites so the flip is
