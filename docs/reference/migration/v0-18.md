...
subscription keeps finding work, for example while it retries a failing
handler, and it still waits up to five seconds for a subscription to start.

### Who is affected

A test that relied on the engine running for a fixed time. For example, a test
that counted on a failed handler being retried after its `retry_delay_seconds`
within that second, while the subscription had nothing else to do. Such a
test now sees fewer retries. Assert on what the engine processed, and run the
retry path directly if the test is about retries.
