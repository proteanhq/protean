...
most `size` entries, and return an empty list for an offset at or past the end.
An adapter that passes `last_position` through as a store cursor no longer meets
the contract.
