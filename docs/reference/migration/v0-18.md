...
Replace `assert_invalid` with `pytest.raises`. Its `message` argument becomes
`match`:
...
`match` is a regular expression, while `message` was a plain substring. Escape
a message that holds characters such as `(`, `.` or `$` with `re.escape`.
