...
handler kept for a retired event can still assign the removed field, and replay
would raise on it under `extra="forbid"`.
...
- `protean ir diff` downgrades the `field_removed` change to safe, cited
  ...
  name is reserved in the new snapshot.
- The declaration has to stay. Taking a name back out of `reserved` on an
  ...
  replay stops dropping an assignment to the name, so a retained handler that
  still writes it raises again.
