...
- **You have a `[lint]` table.** `level`, `suppressions`, `rules` and the other
  ...
  except `level` applies to the IR builder too. A project that passed `protean check` can start failing, for
  example when `level = "info"` makes info findings gate, or when a custom rule
  listed in `rules` reports findings. Run `protean check` and adjust `[lint]`.
- **Your `[lint]` table has a malformed value.** A value that used to be
  ignored is now checked. A `[lint]` that is not a table, a bad `level`, a
  suppression count that is not a non-negative integer, a `rules` value that is
  not a list of strings, a limit that is not a non-negative integer, or a
  `check_*` flag that is not `true` or `false` makes `protean check` exit `2`
  and `protean verify` exit `4`. Every IR build (`protean ir show`, the MCP
  server, the pre-commit hooks) raises `ConfigurationError` for the same values,
  except a bad `level`: the IR builder does not use `level`, so it does not
  check it. Fix the value the error message names.
...
- **You have an `[observatory]` table.** `trace_retention_days` now applies to
  trace persistence. A value that is not a table, or a retention that is not a
  number, falls back to the default of 7 days.

Environment overlays such as `[test.lint]` were already loaded when
`PROTEAN_ENV` named that environment. They are unchanged.
