...
## Removing a field from an event-sourced aggregate needs `reserved` { #removing-a-field-from-an-event-sourced-aggregate }
...
`reserved` does the migration work, in the same family as `renamed_from` and
`deprecated`:
