...
convention (that other commands can converge onto later). Neither command shipped
in a released version with the old
shape (`check`'s JSON shipped in 0.15.0; the diagnostic schema and `[lint].level`
gating in 0.17.0; `verify` is new in this cycle), so this is a pre-release
correction, not a break. You only need to act if you already consume the
unreleased output.
