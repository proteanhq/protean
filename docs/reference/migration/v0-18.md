...
- [`protean server` and `protean observatory` apply `[logging]`](#protean-server-and-protean-observatory-apply-logging): log output follows `domain.toml`, and a plain `protean server` logs at DEBUG in development
...
`[logging]` section of `domain.toml`. The commands set up console logging at
INFO before the domain loaded, and `Domain.init()` then saw handlers on the root
logger and skipped its own setup. Both commands now call
`Domain.configure_logging()` once the domain loads, the way `--reload` and each
multi-worker process already did. Your `level`, `format`, `redact`,
`per_logger`, and sampling settings now take effect, and engine logs carry
`correlation_id`.

`--log-level` and `--log-format` now override only the level and the format.
The rest of `[logging]` still applies. `--log-config` still replaces
everything.
...
output changes on upgrade in two cases:

- **You have a `[logging]` section.** Its settings now apply: a JSON `format`,
  a different `level`, redacted fields.
- **You have no `level` setting.** With no `[logging].level`, no
  `PROTEAN_LOG_LEVEL`, and no `--log-level`, the level now comes from
  `PROTEAN_ENV`. An unset `PROTEAN_ENV` counts as `development`, so a plain
  `protean server` now logs at DEBUG instead of INFO. Set `[logging].level`,
  `PROTEAN_LOG_LEVEL`, or `PROTEAN_ENV=production` to get INFO back.

A multi-worker run also changes: the supervisor now forwards worker records at
the `[logging]` level, so `level = "DEBUG"` shows DEBUG records it used to drop.
