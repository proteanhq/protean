...
`QuerySet.update()` patched every matched row in one call. Load the matched
aggregates, invoke the behaviour method on each, and persist them inside a
`UnitOfWork` so the batch commits or rolls back together.
...
# After: load-and-invoke loop inside a UnitOfWork
repo = domain.repository_for(Order)
with UnitOfWork():
