...
- **You have a `[lint]` table.** `level`, `suppressions`, `rules` and the other
  `[lint]` keys now apply to `protean check`, `protean verify` and the IR
  builder. A project that passed `protean check` can start failing, for
...
- **Your `[lint]` table has a malformed value.** A value that used to be
  ...
  server, the pre-commit hooks) raises `ConfigurationError`. Fix the value the
  error message names.
