...
A negative `last_position` or `size` now raises `ValueError` on both adapters. A
negative value slices from the end under Python's list semantics, so it used to
return a different page than the offset named on the memory cache and does not
belong in a zero-based offset. Pass zero or greater.

If you maintain your own `BaseCache` adapter, `get_all` is now spelled out in the
port docstring and your implementation has to match it: sort the matching keys
ascending, read `last_position` as a zero-based offset into that order, return at
most `size` entries, return an empty list for an offset at or past the end, and
raise `ValueError` for a negative `last_position` or `size`. `BaseCache._page`
does the bounds check and the slice, so an adapter that sorts its keys and hands
them to `_page` gets all of this for free. An adapter that passes `last_position`
through as a store cursor no longer meets the contract.
