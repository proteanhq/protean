...
- [`replay_exhausted` returns a `ReplayOutcome` instead of a bool](#replay_exhausted-returns-a-replayoutcome-instead-of-a-bool): an expired command is refused, and the result is no longer a bool
...
## `replay_exhausted` returns a `ReplayOutcome` instead of a bool

`EventStoreSubscription.replay_exhausted` used to return `True` when the
handler succeeded and `False` when it failed again. It now returns a
`ReplayOutcome`: `RESOLVED`, `REOPENED` or `EXPIRED`.

`EXPIRED` is new. The method now refuses a command whose deadline has passed.
It does not dispatch the command and writes no record, so the position stays
`Exhausted` for you to purge. Before, the engine skipped the expired command
but still reported it handled, and the method recorded the position
`Resolved` although the handler never ran.

### Who is affected

Code that calls `replay_exhausted` directly and tests its result as a bool.
`ReplayOutcome` is a string enum and every member is truthy, so
`if await sub.replay_exhausted(...)` is now always true. Compare against the
member instead:

```python
# Before
if await sub.replay_exhausted(event, position, ...):
    ...

# After
from protean.server.subscription.event_store_subscription import ReplayOutcome

outcome = await sub.replay_exhausted(event, position, ...)
if outcome == ReplayOutcome.RESOLVED:
    ...
elif outcome == ReplayOutcome.EXPIRED:
    ...  # purge the position
```

`protean eventstore dlq replay` needs no change. It already refused an expired
command with a "deadline has passed ... Purge it instead" error. It now also
shows that error when `replay_exhausted` finds the deadline passed after the
CLI's own check. The engine checks the deadline once more when it dispatches.
A deadline that passes between that check and the one in `replay_exhausted`
is still skipped and recorded `Resolved`.
