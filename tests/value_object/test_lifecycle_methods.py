...
def test_that_building_is_marked_as_done_if_above_4_floors():
    building = Building(name="Foo", floors=4)

    assert building.status == BuildingStatus.DONE.value


def test_that_building_is_marked_as_done_if_below_4_floors():
    building = Building(name="Foo", floors=1)

    assert building.status == BuildingStatus.WIP.value


def test_that_building_cannot_be_WIP_if_above_4_floors(test_domain):
