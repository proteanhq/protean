...
recomputes every tool result by re-running the tool.
"""
...
@dataclass
class RunResult:
    """The outcome of one run: the assistant turns, the produced project's hash,
    and the ``run_verify`` results the run recorded, in order."""
...
def replay(transcript: Transcript, *, workspace: Workspace) -> RunResult:
    ...
    still carries tool calls, so a replay cannot loop. The caller compares
    ``result.project_hash`` against ``transcript.project_hash`` to detect a stale
    transcript.
    """
