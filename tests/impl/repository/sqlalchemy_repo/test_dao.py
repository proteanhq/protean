...
class TestDAODeleteFunctionality:
    ...
    def test_deleting_all_records_of_a_type_satisfying_a_filter(self, test_domain):
        ...
        test_domain.get_dao(Person).create(id='2', first_name='Porthos', last_name='Musketeer', age=3)
        test_domain.get_dao(Person).create(id='3', first_name='Aramis', last_name='Musketeer', age=4)
        test_domain.get_dao(Person).create(id='4', first_name='d\'Artagnan', last_name='Musketeer', age=5)
...
class TestDAORetrievalFunctionality:
    ...
    def test_traversal_of_filter_results(self, test_domain):
        ...
        for counter in range(1, 5):
            test_domain.get_dao(Person).create(id=str(counter), first_name=f'John{counter}', last_name='Doe')

        people = test_domain.get_dao(Person).query.limit(2).order_by('id').all()
        assert people is not None
        assert people.total == 4
        assert len(people.items) == 2
...
class TestDAOUpdateFunctionality:
    ...
    def test_that_updating_a_deleted_aggregate_raises_object_not_found_error(self, test_domain):
        ...
        person = test_domain.get_dao(Person).create(id='11344234', first_name='Johnny', last_name='John')
    ...
    def test_updating_record_with_dictionary_args(self, test_domain):
        ...
        person = test_domain.get_dao(Person).create(id='2', first_name='Johnny', last_name='John', age=2)
...
class TestDAOLookup:
    ...
    def test_registration_of_a_lookup_to_an_adapter(self):
        from protean.impl.repository.sqlalchemy_repo import SAProvider
        assert SAProvider.get_lookups().get('sample') == self.SampleLookup
