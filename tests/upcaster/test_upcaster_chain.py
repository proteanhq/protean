...
class TestSingleStepChain:
    ...
    def test_upcast_transforms_data(self, test_domain):
        ...
        result = test_domain._upcaster_chain.upcast("Test.OrderShipped", 1, data)
...
class TestMultiStepChain:
    ...
    def test_v1_upcast_applies_both_steps(self, test_domain):
        ...
        result = test_domain._upcaster_chain.upcast("Test.OrderPlacedV3", 1, data)
    ...
    def test_v2_upcast_applies_one_step(self, test_domain):
        ...
        result = test_domain._upcaster_chain.upcast("Test.OrderPlacedV3", 2, data)
        assert result["total_amount"] == 99.99
...
class TestNoUpcasting:
    ...
    def test_upcast_returns_data_unchanged(self, test_domain):
        ...
        result = test_domain._upcaster_chain.upcast("Test.OrderShipped", 2, data)
        assert result == data
...
class TestCycleDetection:
    def test_error_on_cycle(self, test_domain):
        ...
        test_domain.upcaster(
            CycleA, event_type=OrderShipped, from_version=1, to_version=2
        )
        test_domain.upcaster(
            CycleB, event_type=OrderShipped, from_version=2, to_version=1
        )
...
class TestNonConvergentChain:
    def test_error_on_multiple_terminal_versions(self, test_domain):
        ...
        test_domain.upcaster(
            BranchA, event_type=OrderPlacedV3, from_version=1, to_version=2
        )
        test_domain.upcaster(
            BranchB, event_type=OrderPlacedV3, from_version=10, to_version=3
        )
...
class TestChainDoesNotReachCurrentVersion:
    def test_error_when_terminal_version_has_no_event(self, test_domain):
        ...
        # 1→99, but no event registered as v99
        test_domain.upcaster(
            WrongTarget, event_type=OrderShipped, from_version=1, to_version=99
        )
