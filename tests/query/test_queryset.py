...
class TestCriteriaConstruction:
    ...
    def test_query_construction_with_multiple_exclude_criteria(self, test_domain):
        ...
        assert str(decon_args) == ("(<Q: (NOT (AND: ('last_name', 'John')))>, "
                                   "<Q: (NOT (AND: ('age', 3)))>)")
        assert decon_kwargs == {}
        assert q1._criteria.connector == Q.AND
        assert q2._criteria.negated is False
    ...
    def test_query_construction_with_combined_filter_and_exclude_with_filter_coming_first(self, test_domain):
        # Filter by the last_name
        q1 = test_domain.get_dao(Person).query.filter(last_name='John').exclude(age=3)

        _, decon_args, decon_kwargs = q1._criteria.deconstruct()

        assert str(decon_args) == "(('last_name', 'John'), <Q: (NOT (AND: ('age', 3)))>)"
        assert decon_kwargs == {}
        assert q1._criteria.connector == Q.AND
        assert q1._criteria.negated is False
        assert Q(*decon_args, **decon_kwargs) == q1._criteria

    def test_query_construction_with_combined_filter_and_exclude_with_exclude_coming_first(self, test_domain):
        # Filter by the last_name
        q1 = test_domain.get_dao(Person).query.exclude(age=3).filter(last_name='John')

        _, decon_args, decon_kwargs = q1._criteria.deconstruct()

        assert str(decon_args) == "(<Q: (NOT (AND: ('age', 3)))>, ('last_name', 'John'))"
        assert decon_kwargs == {}
        assert q1._criteria.children[0].connector == Q.AND
        assert q1._criteria.negated is False
        assert Q(*decon_args, **decon_kwargs) == q1._criteria

    def test_query_construction_with_single_Q_object_input_to_filter_method(self, test_domain):
        # Filter by the last_name
    ...
    def test_query_construction_with_single_Q_object_to_exclude_method(self, test_domain):
        # Filter by the last_name
    ...
    def test_query_construnction_with_multiple_Q_objects_input_to_filter_method(self, test_domain):
        # Filter by the last_name
        q1 = test_domain.get_dao(Person).query.filter(Q(last_name='John'), Q(age=3))

        _, decon_args, decon_kwargs = q1._criteria.deconstruct()

        assert str(decon_args) == "(<Q: (AND: ('last_name', 'John'))>, <Q: (AND: ('age', 3))>)"
        assert decon_kwargs == {}
        assert q1._criteria.children[0].connector == Q.AND
        assert q1._criteria.negated is False
        assert Q(*decon_args, **decon_kwargs) == q1._criteria

    def test_query_construnction_with_multiple_Q_objects_input_to_exclude_method(self, test_domain):
    ...
    def test_query_construction_with_multiple_AND_criteria(self, test_domain):
        # Filter by the last_name
        q1 = test_domain.get_dao(Person).query.filter(Q(last_name='John') & Q(age=3) & Q(name='Jean'))
    ...
    def test_query_construction_with_AND_OR_combinations(self, test_domain):
        # Filter by the last_name
        q1 = test_domain.get_dao(Person).query.filter(Q(last_name='John') | Q(age=3), first_name='Jean')
        _, decon_args, decon_kwargs = q1._criteria.deconstruct()
        assert str(decon_args) == ("(<Q: (OR: ('last_name', 'John'), ('age', 3))>, "
                                   "('first_name', 'Jean'))")
        assert Q(*decon_args, **decon_kwargs) == q1._criteria
        ...
        q2 = test_domain.get_dao(Person).query.filter((Q(last_name='John') | Q(age=3)), Q(first_name='Jean'))
        _, decon_args, decon_kwargs = q2._criteria.deconstruct()
        assert str(decon_args) == ("(<Q: (OR: ('last_name', 'John'), ('age', 3))>, "
        ...
        q3 = test_domain.get_dao(Person).query.filter(Q(first_name='Jean') & (Q(last_name='John') | Q(age=3)))
    ...
    def test_list(self, test_domain):
        ...
        test_domain.get_dao(Person).create(id=4, first_name='Bart', age=6, last_name='Carrie')
    ...
    def test_slice(self, test_domain):
        ...
        test_domain.get_dao(Person).create(id=3, first_name='Jean', age=3, last_name='John')
        test_domain.get_dao(Person).create(id=4, first_name='Bart', age=6, last_name='Carrie')
        test_domain.get_dao(Person).create(id=5, first_name='Fred', age=4, last_name='Constantine')
        test_domain.get_dao(Person).create(id=6, first_name='Flint', age=2, last_name='Steve')
    ...
    def test_total_with_cache(self, test_domain):
        ...
        test_domain.get_dao(Person).create(id=5, first_name='Berry', age=1, last_name='John')
        assert query.total == 2
        assert query._result_cache.total == 2
    ...
    def test_has_next_with_cache(self, test_domain):
        ...
        test_domain.get_dao(Person).create(id=2, first_name='Murdock', age=7, last_name='John')
        test_domain.get_dao(Person).create(id=3, first_name='Jean', age=3, last_name='John')
        dog = test_domain.get_dao(Person).create(id=4, first_name='Bart', age=6, last_name='Carrie')
    ...
    def test_first_with_cache(self, test_domain):
        ...
        test_domain.get_dao(Person).create(id=5, first_name='Berry', age=8, last_name='John')
        assert query.first.id == 2
    ...
    def test_raw(self, test_domain):
        ...
        results = test_domain.get_dao(Person).query.raw('{"last_name":"John", "age__in":[6,7]}')
        assert results.total == 1

        results = test_domain.get_dao(Person).query.raw('{"last_name":"John", "age__in":[3,7]}')
        assert results.total == 2
