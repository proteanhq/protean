...
class TestConjunctions:
    ...
    def test_that_kwargs_to_filter_are_ANDed_by_default(
        ...
        q1 = test_domain.repository_for(Person).query.filter(last_name="John", age=3)
        assert q1.total == 1
    ...
    def test_that_kwargs_to_exclude_are_ORed_by_default(
        ...
        q2 = test_domain.repository_for(Person).query.exclude(
            last_name="Carrie", age=10
        )
        assert q2.total == 2
    ...
    def test_empty_resultset_is_returned_correctly(self, test_domain, create_3_people):
        q1 = test_domain.repository_for(Person).query.filter(last_name="XYZ", age=100)
        assert q1.total == 0
