...
def test_for_in_lookup(test_domain):
    ...
    users = user_repo.query.filter(email__in=["foo@example.com"]).all().items
    assert len(users) == 1

    users = (
        user_repo.query.filter(email__in=["foo@example.com", "baz@example.com"])
        .all()
        .items
    )
    assert len(users) == 2

    users = user_repo.query.filter(email__in="qux@example.com").all().items
    assert len(users) == 1
