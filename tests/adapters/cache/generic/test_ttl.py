...
REJECTED_TTLS = [0, -5, "nan", "inf", "not-a-number"]
...
class TestGetTTLOnANeverExpiringKey:
    ...
    cannot (`TTLDict.__setitem__` always assigns one from the cache's
    configured/default TTL). A missing capability, not a disagreement, so
    this is gated to Redis rather than xfailed on memory.
    """
