...
@pytest.mark.redis
def test_dlq_trim_removes_old_entries(redis_broker):
    """dlq_trim removes entries older than min_id."""
    import time

    dlq_stream = "test-dlq-trim:dlq"
    try:
        redis_broker.redis_instance.xadd(dlq_stream, {"data": "old1"})
        redis_broker.redis_instance.xadd(dlq_stream, {"data": "old2"})
        time.sleep(0.01)
        min_id = f"{int(time.time() * 1000)}-0"
        redis_broker.redis_instance.xadd(dlq_stream, {"data": "new1"})

        trimmed = redis_broker.dlq_trim(dlq_stream, min_id)
        assert trimmed >= 2

        remaining = redis_broker.dlq_depth(dlq_stream)
        assert remaining >= 1
    finally:
        redis_broker.redis_instance.delete(dlq_stream)


@pytest.mark.redis
def test_dlq_depth_returns_stream_length(redis_broker):
    """dlq_depth returns the number of entries in a DLQ stream."""
    dlq_stream = "test-dlq-depth:dlq"
    try:
        assert redis_broker.dlq_depth(dlq_stream) == 0

        redis_broker.redis_instance.xadd(dlq_stream, {"data": "msg1"})
        redis_broker.redis_instance.xadd(dlq_stream, {"data": "msg2"})

        assert redis_broker.dlq_depth(dlq_stream) == 2
    finally:
        redis_broker.redis_instance.delete(dlq_stream)


@pytest.mark.redis
def test_dlq_trim_nonexistent_stream(redis_broker):
    """dlq_trim returns 0 for a stream that doesn't exist."""
    result = redis_broker.dlq_trim("nonexistent-dlq-trim:dlq", "999999999999-0")
    assert result == 0


@pytest.mark.redis
def test_dlq_depth_nonexistent_stream(redis_broker):
    """dlq_depth returns 0 for a stream that doesn't exist."""
    result = redis_broker.dlq_depth("nonexistent-dlq-depth:dlq")
    assert result == 0


@pytest.mark.redis
def test_dlq_trim_with_response_error(redis_broker, monkeypatch):
    """dlq_trim returns 0 when Redis raises ResponseError."""
    import redis as redis_module

    def mock_xtrim(*args, **kwargs):
        raise redis_module.ResponseError("some error")

    monkeypatch.setattr(redis_broker.redis_instance, "xtrim", mock_xtrim)

    result = redis_broker.dlq_trim("test:dlq", "0-0")
    assert result == 0
