...
def test_string_repr_and_str():
    ...
    str_obj4 = String(max_length=50, min_length=50, sanitize=True)
    str_obj5 = String(default="John Doe")
    str_obj6 = String(default="John Doe", min_length=50, max_length=50)
    str_obj7 = String(sanitize=False)
    ...
    assert repr(str_obj3) == str(str_obj3) == "String(max_length=255, sanitize=True)"
    assert (
        repr(str_obj4)
        == str(str_obj4)
        == "String(max_length=50, min_length=50, sanitize=True)"
    )
    assert (
...
def test_text_repr_and_str():
    text_obj1 = Text(sanitize=True)
    text_obj2 = Text(default="John Doe")
    text_obj3 = Text(required=True, sanitize=True)
    text_obj4 = Text(default="John Doe")
    text_obj5 = Text(sanitize=False)

    assert repr(text_obj1) == str(text_obj1) == "Text(sanitize=True)"
