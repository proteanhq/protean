...
class TestClusters:
    def test_cluster_count(self, ir):
        # 3 domain aggregates (MemoryMessage is internal, excluded)
        assert len(ir["clusters"]) == 3

    def test_cluster_names(self, ir):
        names = {c["aggregate"]["name"] for c in ir["clusters"].values()}
        assert names == {"Order", "Inventory", "Shipment"}
...
class TestElementsIndex:
    ...
    def test_aggregate_count(self, ir):
        # 3 domain aggregates (MemoryMessage is internal, excluded)
        assert len(ir["elements"]["AGGREGATE"]) == 3
    ...
    def test_event_count(self, ir):
        ...
        # (_OrderFulfillmentTransition is internal/auto-generated, excluded)
        assert len(ir["elements"]["EVENT"]) == 4
