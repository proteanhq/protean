...
def test_one_event_after_persistence(test_domain):
    ...
    assert (
        refreshed_user._events[0]._metadata.id == f"test::user-{refreshed_user.id}-1.1"
    )
...
def test_multiple_events_after_persistence(test_domain):
    ...
    assert refreshed_user._events[0]._metadata.sequence_id == "1.1"
    assert (
        refreshed_user._events[1]._metadata.id == f"test::user-{refreshed_user.id}-1.2"
    )
...
def test_multiple_events_after_multiple_persistence(test_domain):
    ...
    assert (
        refreshed_user._events[0]._metadata.id == f"test::user-{refreshed_user.id}-2.1"
    )
    assert refreshed_user._events[0]._metadata.sequence_id == "2.1"
    assert (
        refreshed_user._events[1]._metadata.id == f"test::user-{refreshed_user.id}-2.2"
    )
