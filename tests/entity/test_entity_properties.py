...
class TestProperties:
    def test_conversion_of_aggregate_values_to_dict(self):
        ...
        assert person.to_dict() == {'id': 12, 'first_name': 'John', 'last_name': 'Doe', 'age': 21}
...
class TestIdentity:
    ...
    def test_non_default_auto_id_field_construction(self):
        ...
        assert PersonAutoSSN.meta_.id_field == PersonAutoSSN.meta_.declared_fields['ssn']
    ...
    def test_non_default_explicit_id_field_construction(self, test_domain):
        ...
        assert PersonExplicitID.meta_.id_field == PersonExplicitID.meta_.declared_fields['ssn']
...
class TestEquivalence:
    ...
    def test_that_two_entities_with_same_id_are_treated_as_equal(self):
        ...
        assert person1 == person3  # Because it's the same record even though attributes differ
    ...
    def test_generated_aggregate_hash(self):
        ...
        person = Person(id=12345, first_name='John', last_name='Doe')
        assert hashed_id == hash(person)  # FIXME Should hash be based on ID alone, or other attrs too?
