"""Tests for the repository `query` property and `find_by` method.

Validates that custom repository methods can use `self.query` and
`self.find_by` instead of reaching through `self._dao`.
"""
