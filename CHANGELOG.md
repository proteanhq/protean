...
- Add `protean.correlation_id` and `protean.causation_id` span attributes to `protean.handler.execute`, `protean.uow.commit`, and `protean.outbox.process` / `protean.outbox.publish` OTEL spans. This enables filtering all spans in a request chain by correlation ID in Jaeger, Datadog, or Tempo.
