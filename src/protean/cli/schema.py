...
def write_index_ddl(
    ...
    # ``load_domain`` returns an initialised domain but leaves no context
    # pushed, and the MySQL key-width guard reads the domain's
    # ``identity_type`` off ``current_domain`` to size an identifier column.
    with domain.domain_context():
        for element_type in ("AGGREGATE", "ENTITY", "PROJECTION"):
            for record in registry._elements.get(element_type, {}).values():
                ...
                cluster = short_name(_fully_qualified_name(element_cls))

                for dialect in dialects:
                    # Dedupe per (schema_name, dialect): per-provider clones
                    # (e.g. the outbox) share a schema name and produce
                    # identical DDL.
                    ...
                    header = (
                        ...
                        f"-- Element: {_fully_qualified_name(element_cls)}\n"
                        f"-- Dialect: {dialect}\n\n"
                    )
