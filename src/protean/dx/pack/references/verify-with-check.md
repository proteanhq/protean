...
everything it reports before you treat the work as done. This step is the same
for every skill, so it lives here once.

1. From the project root, run `protean check --domain=<your_domain>`. It loads
   the domain, validates it, and prints a diagnostic for every problem it finds,
   each with a code.
