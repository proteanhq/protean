...
class BaseDAO(metaclass=ABCMeta):
    ...
    def _filter(
        ...
        When ``projection`` is a list of attribute (column) names, adapters
        should fetch only those columns (the identifier is always among them)
        instead of full rows. The returned ``ResultSet.items`` are still raw
        storage records; the caller builds read-only ``Record`` objects from them
        via the model's ``to_records``. ``None`` means fetch full rows as usual.
        """
