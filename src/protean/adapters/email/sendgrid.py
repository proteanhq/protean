...
class SendgridEmailProvider(BaseEmailProvider):
    ...
    def send_email(self, email_message: BaseEmail) -> bool:
        ...
        email = Mail(
            from_email=email_message.from_email or self.conn_info["DEFAULT_FROM_EMAIL"],
            to_emails=email_message.to,
        )
        email.dynamic_template_data = email_message.data
        email.template_id = TemplateId(email_message.template)
