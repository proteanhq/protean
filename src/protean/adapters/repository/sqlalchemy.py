...
class SADAO(BaseDAO):
    ...
    def _filter(
        ...
        # Restrict the columns loaded to the projection (plus the primary key,
        # which SQLAlchemy always loads). Large/unneeded columns — JSON blobs in
        # particular — are left out of the SELECT entirely. The mapped attribute
        # honours ``referenced_as`` so it matches the key ``to_records`` reads.
    ...
    def _count(self, criteria: Q) -> int:
        """Count rows matching ``criteria`` via ``SELECT COUNT(*)``.

        Avoids the wrapped subquery generated by ``query.count()`` on a
        full-entity query by using ``query(func.count())`` directly.
        """
        conn = self._get_session()
        assert conn is not None
        try:
            qs = conn.query(func.count()).select_from(self.database_model_cls)
            if criteria.children:
                qs = qs.filter(self._build_filters(criteria))
...
class IsNull(DefaultLookup):
    ...
    ``Q(field__isnull=True)`` emits ``field IS NULL``;
    ``Q(field__isnull=False)`` emits ``field IS NOT NULL``.
    """
